...
impl Config 
{
    pub fn from_env() -> Result<Self, ConfigError> 
    {
        ...
        let db_url = std::env::var("DATABASE_URL").map_err(|_| ConfigError::Missing("DATABASE_URL".to_string()))?;
        ...
        let container_memory_mb = std::env::var("DOCKER_CONTAINER_MEMORY_MB").ok()
            .and_then(|s| s.parse().ok()).unwrap_or(512);

        let container_cpu_quota = std::env::var("DOCKER_CONTAINER_CPU_QUOTA").ok()
            .and_then(|s| s.parse().ok()).unwrap_or(50000);

        let db_max_connections = std::env::var("DB_MAX_CONNECTIONS").ok()
            .and_then(|s| s.parse().ok()).unwrap_or(10);
            
        let timeout_normal = std::env::var("TIMEOUT_SECONDS_NORMAL").ok()
            .and_then(|s| s.parse().ok()).unwrap_or(10);

        let timeout_long = std::env::var("TIMEOUT_SECONDS_LONG").ok()
